...
async function bootstrap() {
  ...
  const config = new DocumentBuilder()
    ...
    .addTag(
      "Provider",
      "These are the methods that send the information of the provider to the hoster as well as the (un)insstallation of the provider to the hoster"
    )
    .addTag(
      "Product",
      "Every call that adds/edits/deletes products on the provider to be used by the HOSTER"
    )
